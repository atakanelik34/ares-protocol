...
| ECO-06 | Governance capture through concentrated voting power | P3-P4 | Launch-critical risk if token distribution and quorum settings permit capture | Governor/timelock model exists; actual capture-cost analysis not complete | Blocked |
| ECO-07 | Flash-loan voting amplification | P4 | Depends on snapshot semantics and token distribution mechanics | Framework assumes snapshot protection; executable governance cert pack missing | Blocked |
